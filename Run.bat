...
REM Fast path: everything's already set up (true on every run after the
REM first) - skip straight to a silent launch, no console window at all.
...
:setup
REM First run (or something's missing) - this is the only time a console
REM window appears, since installing prerequisites needs to be watchable.
echo ============================================
echo  Game Scheduler - first-time setup
echo ============================================
echo.
...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        ...
        echo this window and double-click Run.bat again to continue.
        echo.
        pause
        exit /b 1
    )
...
echo Installing the app-window library ^(pywebview^) - this gives the app a real
...
echo.
echo ============================================
echo  Setup complete - opening the app now. From now on, just double-click
echo  Run.bat and it opens straight away, no setup screen.
echo ============================================
timeout /t 2 >nul

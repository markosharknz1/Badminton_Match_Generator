...
const { peakConcurrent, ageCategory, sessionReportRows, attendanceCounts, paymentBreakdown } = require('./sessionReport');
...
async function main() {
    ...
    await test('payment breakdown groups by category with count and total funds, over all checked-in players regardless of whether they played', async (db) => {
        ...
        assert.deepStrictEqual(r.payment_breakdown, [
            { category: 'Card', count: 1, amount_cents: 1500, methods: [], players: [{ player_id: 3, first_name: 'P3', last_name: 'X', method: null, amount_cents: 1500 }] },
            {
                category: 'Cash', count: 2, amount_cents: 2000, methods: [],
                players: [
                    { player_id: 1, first_name: 'P1', last_name: 'X', method: null, amount_cents: 1000 },
                    { player_id: 2, first_name: 'P2', last_name: 'X', method: null, amount_cents: 1000 },
                ],
            },
        ]);
        ...
        assert.deepStrictEqual(r.payment_breakdown, [
            {
                ...
                players: [
                    { player_id: 1, first_name: 'P1', last_name: 'X', method: 'Cash', amount_cents: 500 },
                    { player_id: 2, first_name: 'P2', last_name: 'X', method: 'Cash', amount_cents: 500 },
                ],
            },
            {
                category: 'Non-Member', count: 2, amount_cents: 2000, methods: [{ method: 'Card', count: 1, amount_cents: 1000 }],
                players: [
                    { player_id: 3, first_name: 'P3', last_name: 'X', method: 'Card', amount_cents: 1000 },
                    { player_id: 4, first_name: 'P4', last_name: 'X', method: null, amount_cents: 1000 },
                ],
            },
        ]);
    ...
    await test('checked_in, booked, and left are counted separately, total is their sum', async (db) => {
        addPlayer(db, 1); checkIn(db, 1, '2026-07-01 19:00:00', 'here_today');
        addPlayer(db, 2); checkIn(db, 2, '2026-07-01 19:00:00', 'playing');
        addPlayer(db, 3); checkIn(db, 3, '2026-07-01 19:00:00', 'booked');
        addPlayer(db, 4); checkIn(db, 4, '2026-07-01 19:00:00', 'left', 'removed'); // arrived, then removed
        const c = attendanceCounts(db, 1);
        assert.deepStrictEqual(c, { checked_in: 2, booked: 1, left: 1, total: 4 });
    });

    await test('a cancelled booking (no-show) counts toward nothing, not even left', async (db) => {
        addPlayer(db, 1); checkIn(db, 1, '2026-07-01 19:00:00', 'here_today');
        addPlayer(db, 2); checkIn(db, 2, '2026-07-01 19:00:00', 'left', 'no-show'); // booking was cancelled before arriving
        const c = attendanceCounts(db, 1);
        assert.deepStrictEqual(c, { checked_in: 1, booked: 0, left: 0, total: 1 });
    });

    await test('a left player who already paid appears under their own category, not a synthetic row', async (db) => {
        db.run(`INSERT INTO payment_categories (id, name) VALUES (1, 'Member')`);
        addPlayer(db, 1); checkIn(db, 1, '2026-07-01 19:00:00', 'here_today'); pay(db, 1, 1, 1000);
        db.run(`UPDATE attendance SET state = 'left', left_reason = 'removed' WHERE player_id = 1`);
        const r = paymentBreakdown(db, 1);
        assert.deepStrictEqual(r.payment_breakdown, [
            { category: 'Member', count: 1, amount_cents: 1000, methods: [], players: [{ player_id: 1, first_name: 'P1', last_name: 'X', method: null, amount_cents: 1000 }] },
        ]);
    });

    await test('a left player with no payment recorded gets a synthetic "Left early / removed" row, so the table still sums to total', async (db) => {
        addPlayer(db, 1); checkIn(db, 1, '2026-07-01 19:00:00', 'left', 'removed'); // never paid before being removed
        const r = paymentBreakdown(db, 1);
        assert.deepStrictEqual(r.payment_breakdown, [
            { category: 'Left early / removed', count: 1, amount_cents: 0, methods: [], players: [{ player_id: 1, first_name: 'P1', last_name: 'X', method: null, amount_cents: 0 }] },
        ]);
    });

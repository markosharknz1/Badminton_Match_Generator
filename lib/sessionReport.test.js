...
async function main() {
    ...
    await test('payment breakdown groups by category with count and total funds, over all checked-in players regardless of whether they played', async (db) => {
        ...
        assert.deepStrictEqual(r.payment_breakdown, [
            { category: 'Card', count: 1, amount_cents: 1500 },
            { category: 'Cash', count: 2, amount_cents: 2000 },
        ]);
        assert.strictEqual(r.total_funds_cents, 3500);
    });

    await test('payment method breakdown is a separate dimension from category - how they paid, not what type of player', async (db) => {
        ...
        const r = sessionReportRows(db, null, null)[0];
        assert.deepStrictEqual(r.payment_breakdown, [
            { category: 'Member', count: 2, amount_cents: 1000 },
            { category: 'Non-Member', count: 2, amount_cents: 2000 },
        ]);

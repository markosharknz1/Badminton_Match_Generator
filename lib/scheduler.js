...
function tick() {
    ...
    // still technically being open. Compares against LOCAL "today", not UTC
    // - see todayLocalDateStr's comment for why that distinction is a real
    // bug for a club well ahead of UTC (a session stayed "open" for hours
    // past local midnight under the old UTC-based check).
    if (session.date < todayLocalDateStr()) {
    ...
    // Belt-and-suspenders alongside the null phase_ends_at check right
    // below (pausing always nulls it too) - makes explicit that a paused
    // session is never force-advanced, rather than relying on that as a
    // side effect of the timestamp being absent.
    if (session.current_phase === 'paused') return;

...
// The three numbers Tonight's totals' headline breaks the night down into:
...
// coming, haven't arrived/paid yet), and total (their sum) - deliberately
// NOT reusing uniquePlayerCount for "total" here, since that also counts a
// player removed mid-session (state='left', any reason other than
// no-show), which would make total not actually equal checked_in + booked
// and look just as broken as the gap this was built to fix.
function attendanceCounts(db, sessionId) {
    ...
    return { checked_in, booked, total: checked_in + booked };
}

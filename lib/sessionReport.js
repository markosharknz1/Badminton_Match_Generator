...
// Payment method breakdown + total funds for the session - every attendance
// row with a payment recorded, regardless of whether that player went on to
// play (see playedDemographics above for why the split differs).
function paymentBreakdown(db, sessionId) {
    ...
    return { payment_breakdown: rows, total_funds_cents };
}
